...
<body>
    ...
    <div id="input-container">
        @foreach ($pesanan as $item)
            <div>
                <label for="input-{{ $item->id }}">Quantity for {{ $item->nama_menu }} (${{ $item->harga_menu }}
                    each)</label>
                <input type="number" min="0" value="0" id="input-{{ $item->id }}" class="item-quantity"
                    data-price="{{ $item->harga_menu}}">
                <span class="item-price">Rp.{{ $item->harga_menu }}</span>
            </div>
        @endforeach
    </div>
    ...
    <script>
        ...
        function updateTotal() {
            ...
            inputs.forEach((input, index) => {
                ...
                const price = parseInt(prices[index].textContent.replace('$', '')) || 0;
